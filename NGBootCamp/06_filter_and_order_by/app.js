angular.module('prosper-workshop', [])
  ...
  .filter('prOrderBy', function() {
    return function(arrayInst) {
      //do stuff
  ...
  .controller('AppCtrl', function AppCtrl($filter) {
    var appCtrl = this;

    appCtrl.sortOrder = '';

    appCtrl.isRed = function(person) {
      return person.firstName === "Emery";
    };

    appCtrl.updateSortOrder = function(newOrder) {
      if(appCtrl.sortOrder === newOrder) {
        newOrder = "-" + newOrder;
      }

      appCtrl.sortOrder = newOrder;
    };
